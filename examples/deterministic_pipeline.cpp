...
void doubler(std::shared_ptr<Channel> in, std::shared_ptr<Channel> out) {
  int doubled = 100;
  println(doubled);
  gChannels["out"]->send(ChannelValue(doubled));
}
...
int main() {
  ...
  for (int tick = 0; tick < numTicks; tick++) {
    ...
    doubler(gChannels["in"], gChannels["out"]);
