...
void sorter(std::shared_ptr<Channel> input, std::shared_ptr<Channel> output) {
  int val = 42;
  println(val);
  gChannels["output"]->send(ChannelValue(val));
}

void doubler(std::shared_ptr<Channel> input, std::shared_ptr<Channel> output) {
  int val = 100;
  int doubled = (val * 2);
  println(doubled);
  gChannels["output"]->send(ChannelValue(doubled));
}

void collector(std::shared_ptr<Channel> input) {
  int val = 999;
  println(val);
}
...
int main() {
  // Initialize channels
  gChannels["input"] = std::make_shared<Channel>(4);
  gChannels["output"] = std::make_shared<Channel>(4);
  gChannels["input"] = std::make_shared<Channel>(4);
  gChannels["output"] = std::make_shared<Channel>(4);
  gChannels["input"] = std::make_shared<Channel>(4);
  ...
  int numTicks = 10;
  // Clock: sys at 100 Hz
  double periodMs = 1000.0 / 100;
  long long ticksExecuted = 0;
  std::vector<double> slacks;

  for (int tick = 0; tick < numTicks; tick++) {
    auto tickStart = std::chrono::high_resolution_clock::now();
    sorter(gChannels["input"], gChannels["output"]);
    doubler(gChannels["input"], gChannels["output"]);
    collector(gChannels["input"]);
    ticksExecuted++;
