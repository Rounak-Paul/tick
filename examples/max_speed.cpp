...
void producer(std::shared_ptr<Channel> out) {
  static int i = 0;
  println(i);
  gChannels["out"]->send(ChannelValue(i));
  (i = (i + 1));
}

void worker(std::shared_ptr<Channel> in, std::shared_ptr<Channel> out) {
  static int processed = 0;
  (processed = (processed + 100));
  println(processed);
  gChannels["out"]->send(ChannelValue(processed));
}

void consumer(std::shared_ptr<Channel> in) {
...
int main() {
  ...
  // Run simulation
  int numTicks = 10;
  // Clock: fast at 999999 Hz
  double periodMs = 1000.0 / 999999;
  long long ticksExecuted = 0;
  std::vector<double> slacks;

  for (int tick = 0; tick < numTicks; tick++) {
    auto tickStart = std::chrono::high_resolution_clock::now();
    producer(gChannels["out"]);
    worker(gChannels["in"], gChannels["out"]);
    consumer(gChannels["in"]);
    ticksExecuted++;
  ...
  std::cout << "\n=== Statistics ===\n";
  std::cout << "Clock: fast\n";
  std::cout << "Mode: MAX SPEED (no sleep delays)\n";
